...
import com.dixa.twilio.client.voice.{ConferenceRecordingReadRequestExecutor, TwilioClientVoice}
import com.dixa.twilio.client.{voice, ApiException, TwilioClient, TwilioTestConstants}
...
final class ConferenceRecordingReadTest extends TwilioClientTest with Matchers {

  classOf[TwilioClientVoice].getSimpleName when {

    "read recordings" should {

      "Support reading all conference recording from account" in {
        ...
        val expectedPath =
          s"/2010-04-01/Accounts/${TwilioTestConstants.accountSid}/Conferences/$conferenceSid1/Recordings.json?ConferenceSid=${conferenceSid1.twilioString}"
        ...
        val req = ConferenceRecordingReadRequestExecutor.ConferenceRecordingReadRequest.build(
          _.withAccountSid(TwilioTestConstants.accountSid)
            .withConferenceSid(conferenceSid1)
            .build()
        )

        val resultFut: Future[
          Seq[Either[
            voice.ConferenceRecordingReadRequestExecutor.ConferenceRecordingReadException,
            Recording
          ]]
        ] =
      ...
      "Return a Left if credentials are wrong" in {
        ...
        val expected =
          Left(
            ConferenceRecordingReadRequestExecutor.ConferenceRecordingReadException.Api(
  ...
  final class Fixture {
    ...
    val instance: ConferenceRecordingReadRequestExecutor =
      TwilioClient.defaultImpl().voice.conferenceRecordingRead
  }
}
