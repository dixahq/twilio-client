...
private[impl] final class ContentSearchRequestExecutorImpl()(
    implicit override protected val http: HttpExt,
    override protected val materializer: Materializer,
    override protected val executionContext: ExecutionContext
) extends ContentSearchRequestExecutor {
  ...
  override protected def createHttpReq(
  ...
  ): Either[ContentSearchException, HttpRequest] = {
    val params =
    ...
    createHttpRequestFor(s"/${ApiVersion.V2}/Content$queryString", connSettings)
  }

  override protected def mapApiException(apiException: ApiException): ApiExceptionWrapper =
    ContentSearchException.Api(apiException)

  override protected def createUnspecifiedException(
      msg: Option[String],
      cause: Option[Throwable]
  ): UnspecifiedException = ContentSearchException.Unspecified(msg, cause)
  ...
  ): List[Either[ContentSearchException, ContentTemplate]] =
    httpResponse.status match {
      case StatusCodes.OK =>
        Try(ujson.read(responseEntity.toString)) match {
          case Failure(ex) =>
            List(Left(ContentSearchException.Unspecified(ex)))
          case Success(json) =>
            json("contents").arr.map { item =>
              ContentJsonRep
                .parseContentTemplate(item)
                .left
                .map(ContentSearchException.Unspecified(_))
            }.toList
        }
      case _ =>
